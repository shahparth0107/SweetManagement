...
export default function Admin(){
  ...
  const dec1 = async (id) => {
    const { data } = await SweetsAPI.purchase(id, 1)
    setItems(prev => prev.map(s => s._id === id ? data.sweet : s))
  }
  const inc1 = async (id) => {
    const { data } = await SweetsAPI.restock(id, 1)
    setItems(prev => prev.map(s => s._id === id ? data.sweet : s))
  }
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell width={260} align="right">Actions</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {items.map(s => (
                <TableRow key={s._id} hover>
                  ...
                  <TableCell align="right">
                    <Tooltip title="Decrease by 1"><IconButton onClick={()=>dec1(s._id)}><RemoveIcon /></IconButton></Tooltip>
                    <Tooltip title="Increase by 1"><IconButton onClick={()=>inc1(s._id)}><AddIcon /></IconButton></Tooltip>
                    <Tooltip title="Restock custom qty"><IconButton onClick={()=>restock(s._id)}><Inventory2Icon /></IconButton></Tooltip>
                    <Tooltip title="Edit"><IconButton onClick={()=>{ setEditing(s._id); setForm({ name:s.name, description:s.description, price:s.price, category:s.category, imageUrl:s.imageUrl, quantity:s.quantity }) }}><EditIcon /></IconButton></Tooltip>
                    <Tooltip title="Delete"><IconButton color="error" onClick={()=>del(s._id)}><DeleteIcon /></IconButton></Tooltip>
                  </TableCell>
                </TableRow>
              ))}
              {items.length === 0 && (
